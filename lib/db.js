...
async function getAllWallets() {
  return req('GET', 'wallets?select=*&order=created_at.asc&limit=1000');
}

async function upsertWallet(tgId, address, privateKeyEnc) {
  const payload = { tg_id: parseInt(tgId), address, network: 'polygon' };
  // Always include private_key_enc — column is NOT NULL
  payload.private_key_enc = privateKeyEnc || '';
...
module.exports = {
  ...
  // wallets
  getWallet, getAllWallets, upsertWallet, getAllWallets,
  // bets
