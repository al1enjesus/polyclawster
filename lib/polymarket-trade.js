...
async function executeTrade(opts) {
  ...
  // For market orders, set price slightly above current ask for fill guarantee
  // Cap at 0.999 to avoid precision issues
  const limitPrice = Math.min(price + 0.05, 0.999);
  ...
  if (!order || (!order.success && !order.orderID && !order.orderId)) {
    throw new Error('Order rejected by CLOB: ' + JSON.stringify(order || {}).slice(0, 150));
  }
