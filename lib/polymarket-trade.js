...
const CLOB_KEYS_FILE = '/workspace/edge/db/clob-keys.json';

function loadClobKeysFile() {
  try {
    const data = fs.readFileSync(CLOB_KEYS_FILE, 'utf8');
    return JSON.parse(data);
  } catch { return {}; }
}

function saveClobKeysFile(store) {
  try {
    const dir = require('path').dirname(CLOB_KEYS_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(CLOB_KEYS_FILE, JSON.stringify(store, null, 2));
  } catch (e) { console.warn('[trade] Could not save clob-keys.json:', e.message); }
}
 ...
 *  3. Persisted in clob-keys.json (survives restarts)
 *  4. Derive new API key from wallet signature (registers with Polymarket)
 */
async function getApiCredsForWallet(wallet, ClobClient, SignatureType) {
  ...
  // 2. Check if this is the master wallet
  ...
  const store = loadClobKeysFile();
  if (store[addr]?.key) {
    _apiKeyCache.set(addr, store[addr]);
    console.log('[trade] CLOB API key loaded from file for:', addr.slice(0, 10) + '...');
    return store[addr];
  }

  // 4. Derive API key from wallet (registers with Polymarket)
  ...
  // Persist to file + memory cache
  store[addr] = creds;
  saveClobKeysFile(store);
  _apiKeyCache.set(addr, creds);
  console.log('[trade] API key derived and saved:', creds.key?.slice(0, 8) + '...');
  return creds;
}
