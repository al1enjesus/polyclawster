...
async function executeTrade(opts) {
  ...
  // Dynamic import — @polymarket/clob-client is ES Module, must use import()
  var ClobClient, SignatureType, Side, ethers;
  try {
    var clobPkg = await import('@polymarket/clob-client');
  ...
  // Resolve token ID via CLOB getMarket (most reliable)
  let tokenId = null;
  let tokenPrice = null;
  if (conditionId) {
    try {
      ...
      if (mkt && mkt.tokens && mkt.tokens.length > 0) {
        // Find the right token: YES/Up = index 0, NO/Down = index 1
        const sideUpper = (side || 'YES').toUpperCase();
        let token;
        if (sideUpper === 'YES' || sideUpper === 'UP') {
          token = mkt.tokens[0];
        } else {
          token = mkt.tokens[1] || mkt.tokens[0];
        }
        tokenId = token.token_id;
        tokenPrice = parseFloat(token.price || 0);
        console.log('[trade] resolved tokenId for', sideUpper, ':', tokenId.slice(0,20)+'...', 'price:', tokenPrice);
      }
    } catch(e) {
  ...
  // Get current price
  var price = tokenPrice || await getBestAsk(client, tokenId);
  if (!price) price = 0.5;

  // Skip if price > 95¢ (market basically resolved)
  if (price > 0.95) {
    throw new Error('Price too high (' + (price * 100).toFixed(0) + '¢) — market likely resolved');
  }
